...
if [ "$1" = "--help" ]; then 
echo "Cкрипт создает образы GRUB2 для использования на загрузочных носителях
Пересоздавать образы требуется только при обновлении GRUB2 или при
добавлении-удалении модулей в GRUB2.
Готовые сгенерированные образы находятся в каталоге /usr/lib/grub"
echo
echo "Образы будут созданы в текущем каталоге, если все не требуется, 
закоментируйте ненужную команду. 
grubx64.efi - EFI образ 
grub2.eltorito - El Torito образ для ISO"
echo
echo "core.img - образ для загрузочного USB, используется вместе с boot.img
boot.img (из grub2) и core.img (делается grub-mkimage) записываются в MBR флешки
или диска командой grub-bios-setup или dd, примеры (/dev/sdb заменить на свою флешку):"
echo "grub-bios-setup -d. -b /usr/lib/grub/i386-pc/boot.img -c ./core.img /dev/sdb"
echo
echo "или"
echo
echo "dd if=./core.img of=/dev/sdb bs=512 seek=1
...
--config=/usr/share/grub/grub_eltorito.cfg --verbose --prefix=/EFI/BOOT \
...
#part_gpt part_msdos biosdisk disk memdisk fat ntldr lvm exfat ext2 ntfs iso9660 gzio xzio test vbe vga normal search configfile linux linux16 chain loopback echo file halt reboot ls true gfxterm gettext font
