...
part_gpt part_msdos disk fat exfat lvm ext2 ntfs iso9660 normal gzio xzio test search_fs_file multiboot2 configfile linux linux16 chain loopback echo efi_gop efi_uga file halt reboot ls true gfxterm gettext font
...
part_msdos biosdisk disk fat iso9660 test vbe vga multiboot2 normal gzio xzio search_fs_file configfile linux linux16 chain loopback echo file halt reboot ls true gfxterm gettext font

#для USB образа
./grub-mkimage --directory=i386-pc --compression=none \
--verbose --prefix=\(hd0,msdos1\)/EFI/BOOT \
--output=core.img --format=i386-pc \
part_gpt part_msdos biosdisk disk fat lvm exfat ext2 ntfs iso9660 gzio xzio test vbe vga multiboot2 normal search_fs_file configfile linux linux16 chain loopback echo file halt reboot ls true gfxterm gettext font
